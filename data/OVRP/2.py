...
# Assuming OVRPEnv is defined in ovrp_env.py or similar
# from ovrp_env import OVRPEnv
...
env_params = {
    'problem_size': 50,  # Adjust if your problem size is different
    'pomo_size': 50,     # Typically set to the problem size
    'device': torch.device('cpu'),  # Use 'cuda' if GPU is available
}
env = OVRPEnv(**env_params)

# Load the dataset from the .pkl file
path = 'ovrp50_uniform1.pkl'  # Replace with the correct path to your .pkl file
data = env.load_dataset(path, num_samples=1)  # Load one sample for visualization

# Unpack the data
depot_xy, node_xy, node_demand = data

# Since we loaded only one sample, extract it
depot_xy = depot_xy[0].squeeze(0)  # Shape: (2,)
node_xy = node_xy[0]               # Shape: (problem_size, 2)
node_demand = node_demand[0]       # Shape: (problem_size,)

# Plot the depot and customer nodes
plt.figure(figsize=(8, 8))
plt.scatter(depot_xy[0], depot_xy[1], c='red', marker='D', s=100, label='Depot')
plt.scatter(node_xy[:, 0], node_xy[:, 1], c='blue', marker='o', s=50, label='Customers')

# Optionally annotate customer nodes with their indices or demands
for i, (x, y) in enumerate(node_xy):
    plt.annotate(f'{i+1}', (x, y), textcoords="offset points", xytext=(0,5), ha='center')

plt.legend()
plt.xlabel('X Coordinate')
plt.ylabel('Y Coordinate')
plt.title('OVRP Nodes Visualization')
plt.grid(True)
plt.show()
